...
\IEEEauthorblockA{\IEEEauthorrefmark{2}University of Warwick, UK\\
Email: \{xhshi, wxzhang, hjin, zhxke, wusong\}@hust.edu.cn, liganghe@dcs.warwick.ac.uk}
}
...
Although a data processing system often work as a batch processing system, many enterprises deploy such a system as a service, which we call the service-oriented data processing system. It has been shown that in-memory data processing systems suffer from serious memory pressure. The situation becomes even worse for the service-oriented data processing systems due to various reasons. For example, in a service-oriented system, multiple submitted tasks are launched at the same time and executed in the same context in the resources, comparing with the batch processing mode where the tasks are processed one by one. Therefore, the memory pressure will affect all submitted tasks, including the tasks that only incur the light memory pressure when they are run alone. Memory pressure arises as the running tasks produce massive long-living data objects in the limited memory space. These long-living objects incur significant memory and CPU overheads. Some tasks cause the heavy memory pressure because of the operations and dataset they are processing, which in turn affect all running tasks in the system. Our studies show that a task often call several API functions provided by the in-memory processing frameworks and most API functions generate long-living data objects in memory with their own regular models. Some tasks need the constant memory space, while others need the linear memory space. As different models have different impact on memory pressure, we propose a method to classify the models that the tasks belong to. The method uses the memory usage rate as the classification criteria. Further, we design a scheduler called MURS to mitigate the memory pressure. We implement MURS in Spark and conduct the experiments to evaluate the performance of MURS. The results show that when comparing to Spark, our scheduler can 1) decrease the execution time of the submitted jobs by up to 65.8\%, 2) mitigate the memory pressure in the server by decreasing the garbage collection time by up to 81\%, and 3) reduce the data spilling, and hence disk I/O, by approximately 90\%. 
