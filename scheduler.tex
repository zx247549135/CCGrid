...
%When the memory pressure is high, we will reduce the value.
...
\Fn{\CST}{
  ...
  \While{$freeMemory > 0$}{
    ...
    $memNecessary \leftarrow comsum[taskId] * (1-percent[taskId])$\;
...
\Fn{\CS}{
  ...
  \Else{
      $memNecessary \leftarrow comsum[taskId] / percent[taskId]$\;
